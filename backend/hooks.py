...
HEBREW_FONT_URL = "https://github.com/googlefonts/noto-fonts/raw/main/hinted/ttf/NotoSansHebrew/NotoSansHebrew-Bold.ttf"
FONT_DIR = "fonts"
FONT_PATH = os.path.join(FONT_DIR, "NotoSerif-Bold.ttf")
HEBREW_FONT_PATH = os.path.join(FONT_DIR, "NotoSansHebrew-Bold.ttf")
...
def download_font_if_needed():
    """Downloads serif and Hebrew fonts for hook text if not present."""
    if not os.path.exists(FONT_DIR):
        os.makedirs(FONT_DIR)
    for url, path in [(FONT_URL, FONT_PATH), (HEBREW_FONT_URL, HEBREW_FONT_PATH)]:
        if not os.path.exists(path):
            print(f"⬇️ Downloading font from {url}...")
            try:
...
def create_hook_image(text, target_width, output_image_path="hook_overlay.png", font_scale=1.0):
    ...
    chosen_font_path = HEBREW_FONT_PATH if _is_hebrew(text) else FONT_PATH
    try:
        font = ImageFont.truetype(chosen_font_path, font_size)
    except Exception as e:
        print(f"⚠️ Warning: Could not load font {chosen_font_path}, using default. Error: {e}")
