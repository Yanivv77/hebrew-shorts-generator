...
DEFAULT_VOICES = {
    "Noa (Female, Hebrew)": "hebrew_female_voice_id",   # replace with ElevenLabs Hebrew female voice ID
    "Amir (Male, Hebrew)": "hebrew_male_voice_id",      # replace with ElevenLabs Hebrew male voice ID
...
def generate_scripts(
    ...
    lang_name = "Hebrew" if language == "he" else ("Spanish" if language == "es" else "English")
    ...
    if language == "he":
        lang_instructions = """
LANGUAGE: ALL narrations, subtitles, captions, and hashtags MUST be in HEBREW.
Use natural casual Hebrew like a real person would speak on TikTok. Contractions, slang OK.
Examples of Hebrew UGC hooks: "רגע, לא ידעתם שזה קיים?...", "תפסיקו לבזבז שעות על...", "לא האמנתי שמשהו כזה קיים..."
Write right-to-left. Keep narrations punchy and conversational.
IMPORTANT: actor_description must remain in ENGLISH (used for image generation). Only narration/captions in Hebrew.
"""
    elif language == "es":
        lang_instructions = """
...
- CTA MUST always mention "link in bio" / "enlace en la bio" / "הקישור בביו". Examples: "Link in bio, go try it", "Check the link in my bio", "El enlace está en la bio, probadlo", "הקישור בביו, תנסו בחינם"
